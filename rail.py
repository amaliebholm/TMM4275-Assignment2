...
dfaPath = "K:\\Biblioteker\\Dokumenter\\Skole\\Automatisering\\TMM4275-Assignment2\\DFAs\\"
...
# Replaces the template with the customers chair values
rail = rail.replace("Rail_template (ug_base_part)",
                    "Rail_Order (ug_base_part)")
# First we need to insert the start position for our rail
rail = rail.replace("<STARTX>", str(startx))
...
# In this case z is the coordinate of the lowest point of the rail_profile
def add_roof_mount(index, x, y, z, roof, string):
...
pointlist = np.array([[0, 0, 0], [0, 10000, 0], [-2000, 12000, 0], [-10000,
                                                                    12000, 0], [-11414.21356, 11414.21356, 0], [-13414.21356, 9414.21356, 0]])
...
r = 2000
element_string = " "
# variable for the start of the line that came before the arc, this variable will be change whenever a new line is implemented
prev_line_start = np.array(pointlist[0])
# variable for the end of the line that comes after the arc, this variable will be change whenever a new line is implemented
...
for i in range(len(pointlist)-1):
    if i == 0 or i/2 == np.round(i/2):
        ...
        print("---")
        rail = add_line(i, pointlist[i][0], pointlist[i][1], pointlist[i][2],
                        pointlist[i+1][0], pointlist[i+1][1], pointlist[i+1][2], rail)
    # arcs
    # Temp solution to only get arc, here the arc and lines are every other time
        ...
        # x start-position for previous line
        xprev_line_start = prev_line_start[0]
        # y start-postion for previous line
        yprev_line_start = prev_line_start[1]
        ...
        side = (xe-xprev_line_start)*(ys-yprev_line_start) - \
            (ye-yprev_line_start)*(xs-xprev_line_start)
        ...
        if (dir != 0):
            ...
            next_end_distance1 = np.sqrt(
                (xnext_line_end-xcenter1)**2 + (ynext_line_end-ycenter1)**2)
            next_end_distance2 = np.sqrt(
                (xnext_line_end-xcenter2)**2 + (ynext_line_end-ycenter2)**2)
        ...
        # The end position for the previous line(which is the curves start) minus its start position in atan2 gives us the angle in
        angle_in_r = m.atan2((ys-yprev_line_start), (xs-xprev_line_start))
        # The end position for the next line minus its start position(which is the curves end) in atan2 gives us the angle out
        angle_out_r = m.atan2((ynext_line_end-ye), (xnext_line_end-xe))

        # Radians we need degrees for DFA file
        print("Radian angle in: " + str(angle_in_r))
        # If the angles surpass 180 degress (meaning it goes downwards) the angle will be negative!!
        print("Radian angle out: " + str(angle_out_r))
        ...
        # Since DFA arcs can not go from a larger angle (e.g. 352) to a smaller angle(e.g.20), we gotta pull some tricks and reverse them or add 360 degrees
        print("----DFA file arc angles----")
        ...
        if dir != 0:  # Here we change the angles from the reference frame to the arc angles in the dfa file
            # Here the curve is mostly going vertically contra sideways on entry
            if abs(going_up/going_right) >= 1 and ye-ys > 0:
                if going_up >= 0 and turn_right_refframe >= 0:  # up and turn towards right
                    if angle_in_d < 90:
                        # we got to swap in and output angles since the arc only move right
                        angle_in = angle_out_d + 90 - 360
            ...
            # Here the curve is mostly going sideways contra vertically on entry
            elif abs(going_up/going_right) < 1:
        ...
        # Temp set to 0 if this is correct, thinking maybe z = pointlist[i][2] is for the roof mounting
        rail = add_arc(i, r, angle_in, angle_out, xcenter, ycenter, 0, rail)
    else:
...
for i in range(len(mount_list)):
    rail = add_roof_mount(
        i, mount_list[i][0], mount_list[i][1], mount_list[i][2], mount_list[i][3], rail)
...
# Saves the customers chair as a new DFA file with the name My_Chair_Order.dfa
